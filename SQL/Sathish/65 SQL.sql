...
--20. List out the employee details according to their last_name in ascending order and salaries in descending order 
	SELECT * FROM EMP ORDER BY LastName ASC, 
	SAL DESC
...
-- 51.How many employees who are working in different departments and display with department name. 
	SELECT D.DNAME, COUNT(EMPNO)AS NOOFEMPOLYESS FROM EMP E RIGHT JOIN DEPT D ON E.DEPTNO=D.DEPTNO GROUP BY DNAME
-- 52.How many employees who are working in sales department.
	SELECT 'SALES' AS DPETNAME,COUNT(EMPNO)AS NOOFEMPLOYEES FROM EMP E JOIN DEPT D ON E.DEPTNO=D.DEPTNO WHERE DNAME='SALES'
...
-- 57.List out the no. of employees on grade wise. 
	SELECT S.GRADE,COUNT(*)AS NOOFEMPLOYEES FROM EMP E JOIN SALGRADE S on E.SAL BETWEEN S.LOSAL AND S.HISAL GROUP BY GRADE
-- 58.Display the employee salary grades and no. of employees between 2000 to 5000 range of salary.
	SELECT S.GRADE, COUNT(*)AS NOOFEMPLOYEES FROM EMP E JOIN SALGRADE S ON E.SAl BETWEEN S.LOSAL AND S.HISAL WHERE GRADE = 4 GROUP BY GRADE
...
-- 60.Display the employee details who earn more than their managers salaries.
	SELECT E.EMPNO,E.FirstName,E.LastName,E.SAL AS EMPSAL,M.SAL AS MGRSAL FROM EMP E JOIN EMP M ON E.MGR = M.MGR WHERE E.SAL>M.SAL
...
--65.List out the common jobs in Research and Accounting Departments in ascending order.
	SELECT JOB,DNAME FROM EMP E INNER JOIN DEPT D ON E.DEPTNO= D.DEPTNO WHERE DNAME='RESEARCH' OR DNAME='ACCOUNTING'